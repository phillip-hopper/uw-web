...
var uwGrabAvailableTexts = function() {
  ...
  function grabContent() {
    display('Grabbing content from ' + uwObject.catalogUrl + '.');
  ...
  function parseBibleData(bibleData) {
    ...
    for (var i = 0; i < bibleData.length; i++) {
      ...
      for (var n = 0; n < versions.length; n++) {
         ...
         */
        mkdirp(uwObject.destinationFolder + '/' + versionInfo.id);
        /**
         * let's add the info.json file
         */
        fileSystem.writeFile(uwObject.destinationFolder + '/' + versionInfo.id + '/info.json', JSON.stringify(versionInfo), function(error) {
          if (error) {
            display('Unable to create the info JSON file at: ' + uwObject.destinationFolder + '/' + versionInfo.id + '/info.json because: ' + error);
          }
        });
        /**
        ...
        fileDownload.dest(uwObject.destinationFolder + '/' + versionInfo.id).run();
      }
    }
  }
  /**
